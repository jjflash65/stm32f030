/*
  ...
  Modification for chip combination STM32F030 and CH340G by R. Seelig 20.12.2016
...
int main(int argc, char* argv[])
{
	...
	fprintf(diag, "f103flash " VERSION "\n\n");
        fprintf(diag, "Original authors of stm32flash:\n"                               \
                      "   (2010) Geoffrey McRae, (2011) Steve Markgraf\n"               \
                      "   (2012-2016) Tormod Volden, (2013-2016) Antonio Borneo\n\n");
	fprintf(diag, "   Original source in: http://stm32flash.sourceforge.net/\n\n");
        fprintf(diag, "   changed version for use with STM32F103 & CH340G  (2017) by R. Seelig\n");
        fprintf(diag, "--------------------------------------------------------------------------\n\n");
	...
	if (action == ACT_WRITE) {
        ...
        fprintf(stderr, "activate bootloader\n\n");
        if ((fd = open(port_opts.device,O_RDWR)) < 0)
        {
        ...
        ioctl(fd, TIOCMGET, &status);      // Status Serielle Schnittstelle
          ...
          ioctl(fd, TIOCMSET, &status);      // setzen
          fflush(stdout);
          usleep(100000);
        ...
        for (pulse= 0; pulse< 3; pulse++)
        {
          ...
          status |= TIOCM_RTS;               // RTS
          ioctl(fd, TIOCMSET, &status);      // loeschen
          fflush(stdout);
          usleep(1000);
